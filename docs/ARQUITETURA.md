...
| Fase                         | Objetivo                    | Entregáveis principais                                                                                                               | Gate                                            |
| ---------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- |
| **0 — Fundação**             | Esqueleto do repo           | Monorepo pnpm, Fastify, git flow, CI (lint+test+coverage 85%), Docker Compose (postgres-core), schema+migração do CORE, health check | CI verde                                        |
| **1 — Tenancy core**         | Resolução dinâmica de banco | Middleware de subdomínio, `TenantConnectionRegistry`, `TenantContext`, cifra de credenciais, 1º banco de tenant conectando           | Request resolve tenant e lê banco isolado (E2E) |
| **2 — Auth global**          | Login e sessão              | Argon2, lockout, JWT RS256+JWKS, sessão única, guards                                                                                | Cobertura ≥85%, login E2E                       |
| **3 — Sistema de módulos**   | Módulos plugáveis           | `ModulesRegistry`, dynamic modules, guard `module_not_enabled`, navegação dinâmica no frontend                                       | Módulo habilita/desabilita por tenant           |
| **4 — Provisionamento**      | Criar tenant fim-a-fim      | `tenant-migrator`, `POST /provisionar` (cria banco+migra+seed), `db_placement` (Strategy)                                            | Novo tenant provisionado do zero                |
| **5 — 1º módulo de produto** | Valor real                  | MasterFila como módulo (ou integração — §9), operando no banco do tenant                                                             | Módulo funcional isolado por tenant             |
| **6 — Licenciamento & BI**   | Planos e painel             | Planos/limites, painel consolidado (BI por push dos módulos)                                                                         | Entitlement barra feature; dashboard com dados  |
