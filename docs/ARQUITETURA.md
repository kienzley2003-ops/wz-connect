...
| **4 — Provisionamento**      | Criar tenant fim-a-fim      | `tenant-migrator`, `POST /tenants/:id/provisionar` (cria banco+migra+seed), `db_placement` (Strategy)                                            | Novo tenant provisionado do zero                | ✅ concluída (2026-07-14)                                                                                                             |
| **5 — 1º módulo de produto** | Valor real                  | `@wz/connect-sdk` (identidade + licenciamento), role de banco dedicada por tenant, credencial revelada no provisionamento, console do MasterFila | Módulo funcional isolado por tenant             | 🟡 **lado do Connect concluído** (2026-07-14): SDK + isolamento de credencial provados; falta a integração no repo do MasterFila (§9) |
| **6 — Licenciamento & BI**   | Planos e painel             | Planos/limites, painel consolidado (BI por push dos módulos — ADR-013)                                                                           | Entitlement barra feature; dashboard com dados  | ✅ concluída (2026-07-14)                                                                                                             |
